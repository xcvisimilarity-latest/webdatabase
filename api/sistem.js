...
async function robustFetch(url, options = {}) {
    ...
    try {
        console.log(`🌐 Fetching: ${url} (timeout: ${timeoutMs}ms)`);
        
        const response = await fetch(url, {
            method: 'GET',
            headers: {
                'User-Agent': 'XCVI-Database-System/4.0.0',
        ...
        console.log('📄 Raw response (truncated):', text ? text.substring(0, 200) : '<empty>');
...
function normalizeJSON(text) {
    ...
    try {
        ...
        try {
            ...
            // Extract JSON array/object if possible
...
// ===== USER STATISTICS =====
async function getUserStatistics(options = {}) {
    const retries = options.retries || 3;
    const retryDelay = options.retryDelayMs || 700;

    try {
        console.log('📊 Fetching user statistics from:', REMOTE_USERS_URL);

        // Cache busting for GitHub raw content
        let url = REMOTE_USERS_URL;
        if (url.includes('raw.githubusercontent.com')) {
            url += (url.includes('?') ? '&' : '?') + 't=' + Date.now();
            console.log('🔄 Using cache-busted URL:', url);
        }
        ...
        for (let attempt = 1; attempt <= retries; attempt++) {
            try {
                const textData = await robustFetch(url, { timeout: 10000 });
                ...
                if (Array.isArray(userData)) {
                    ...
                    console.log(`✅ User stats fetched: ${stats.totalUsers} total, ${stats.premiumUsers} premium`);
                    return stats;
                } else {
        ...
        console.error('❌ All attempts to fetch user stats failed:', lastError?.message);
        return userStats; // Return cached stats as fallback
        
    } catch (error) {
        console.error('❌ Unexpected error in getUserStatistics:', error.message);
        return userStats; // Return cached stats as fallback
    }
}
...
async function createUserAccount(username, clientIp) {
    ...
    // Forward to remote creation endpoint
    ...
    for (let attempt = 1; attempt <= FORWARD_RETRIES; attempt++) {
        try {
            const abortController = new AbortController();
            const timeoutId = setTimeout(() => abortController.abort(), FORWARD_TIMEOUT);
            
            const response = await fetch(REMOTE_CREATE_URL, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'User-Agent': 'XCVI-Database-System/4.0.0'
            ...
            if (response.ok) {
                console.log('✅ Remote account creation successful');
                return {
                    success: true,
                    account: accountData,
                ...
                console.warn(`⚠️ Remote creation failed (attempt ${attempt}):`, response.status, responseData);
                lastForwardError = {
            ...
            clearTimeout(timeoutId);
            console.warn(`⚠️ Forward attempt ${attempt} failed:`, error.message);
    ...
    throw new Error(`Remote creation failed after ${FORWARD_RETRIES} attempts: ${lastForwardError?.message || 'Unknown error'}`);
}
...
module.exports = async (req, res) => {
    ...
    console.log('👤 Client IP:', req.headers['x-forwarded-for'] || req.headers['x-real-ip'] || req.socket?.remoteAddress);
    ...
    try {
        ...
        res.setHeader('X-Powered-By', 'XCVI-Database-System/4.0.0');
        ...
        if (req.method === 'POST') {
            ...
            if (!validation.valid) {
                await sendTelegramNotification(
                    `🚫 <b>Request Blocked</b>\n` +
                    `📱 <b>IP:</b> <code>${ipFromReq(req)}</code>\n` +
